...
import { earnTrend } from "@/lib/platformData";
...
type EarnScreenProps = {
  ...
  smeGigs: { name: string; need: string; pay: string; loc: string; abbr: string }[];
  payouts: { title: string; date: string; amount: string; status: string; tone: "pos" | "warn" }[];
};
...
export default function EarnScreen({ kpis, gigs, smeGigs, payouts }: EarnScreenProps) {
          ...
          <div className="pf-card pf-pad">
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 11 }}>
              {gigs.map((g) => {
                ...
                return (
                  <div key={g.id} className="pf-gig-row">
